...
\begin{split}
\eta_{ij}
&= \beta_{0} + \beta_{1}Treatment_{i} + \beta_{2}Month_{ij} +\beta_{3}Month_{ij}^2+ \beta_{4}(Treatment_{i} * Month_{ij}) + \beta_{5}(Treatment_{i}*Month_{ij}^2 )\\
\end{split}
$$
...
```{r}
# GEE Model 1: 
# - quadratic trend with Month
# - interaction with Treatment 
gee1 = geeglm(Response ~ Treatment * (Month + I(Month^2)), data = toenail1, family = "binomial"(link = "logit"), id = Subject_ID, corstr = "exchangeable")
summary(gee1)
```
...
\begin{split}
\eta_{ij}
&= \beta_{0} + \beta_{1}Treatment_{i} + \beta_{2}Month_{ij} +\beta_{3}Month_{ij}^2\\
\end{split}
$$
```{r}
# GEE Model 2: 
# - quadratic trend with Month
# - interaction with Treatment 
gee2 = geeglm(Response ~ Treatment + (Month + I(Month^2)), data = toenail1, family = "binomial"(link = "logit"), id = Subject_ID, corstr = "exchangeable")
summary(gee2)
```
```{r}
sum_1 = summary(gee2) 
...
\begin{split}
\eta_{ij}
&= -0.399 -0.007Treatment -0.326Month +0.012Month^2 \\
\end{split}
$$

* $\beta_{0}$:Log odds of moderate or severe onycholysis for oral treatment B at baseline month is -0.399
* $\beta_{1}$:Log odds ratio of moderate or severe onycholosis comparing treatment A to treatment B with month fixed is -0.007
* $\beta_{2}$:Log odds ratio of moderate or severe onycholosis for one unit increase in month among treatment B is -0.326
* $\beta_{3}$:This is a non-linear term and it is difficult to interpret
...
```{r}
gee3 = geeglm(Response ~ Treatment*(Month + I(Month^2)), data = toenail1, family = "binomial"(link = "logit"), id = Subject_ID, corstr = "unstructured")
summary(gee3)
```
...
As we can see from the result, the p-value is less than 0.05 so that when the correlation structure is unstructured,the analysis and inference are sensitive to this choice.

Correlation structure:independence
```{r}
gee4 = geeglm(Response ~ Treatment*(Month + I(Month^2)), data = toenail1, family = "binomial"(link = "logit"), id = Subject_ID, corstr = "independence")
summary(gee4)
```
...
```{r}
gee5 = geeglm(Response ~ Treatment*(Month + I(Month^2)), data = toenail1, family = "binomial"(link = "logit"), id = Subject_ID, corstr = "ar1")
summary(gee5)
```
...
## 5.Do you need to account for overdisperion.Comment

The scale parameter for the models are : for model in part 1) the scale parameter is `summary(gee_q2)$dispersion[1]` and the for the model in part 4) the scale parameter is `summary(gee_q2_4)$dispersion[1]`. Both of them are greater than 1,so that we need to account for overdispersion.
